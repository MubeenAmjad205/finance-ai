...
export class NeonPostgresClient {
  ...
  constructor(env: Env) {
    ...
    if (
      ...
      try {
        if (!cachedSqlClient || cachedConnStr !== connectionString) {
          cachedSqlClient = neon(connectionString);
          cachedConnStr = connectionString;
        }
        this.sqlClient = cachedSqlClient;
  ...
  async query<T = any>(queryText: string, params: any[] = []): Promise<T[]> {
    ...
    try {
      ...
      const fn = typeof this.sqlClient.query === 'function' ? this.sqlClient.query.bind(this.sqlClient) : this.sqlClient;
      const rows = await fn(queryText, params);
      this.lastError = null;
  ...
  private async ensureSchema(): Promise<void> {
    if (schemaEnsured || !this.sqlClient) return;
    schemaEnsured = true;
    ...
    try {
      const fn = typeof this.sqlClient.query === 'function' ? this.sqlClient.query.bind(this.sqlClient) : this.sqlClient;
      await Promise.all(tables.map(ddl => fn(ddl)));
    } catch (err: any) {
